...
try:
    ...
    logger.warning("セキュリティミドルウェアが利用できません")
...
# Flaskアプリケーションの初期化
app = Flask(__name__, static_folder=None)  # 静的ファイル配信を無効化
...
@security_middleware if SECURITY_MIDDLEWARE_AVAILABLE else lambda f: f
def health_check():
    """
    ヘルスチェックエンドポイント
    APIサーバーの稼働状況を確認
